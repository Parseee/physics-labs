...
$ va(F) = m va(g) $
$ va(F_c) = -k va(v) = vecrow(-k v_x, -k v_y) , k > 0, "для линейного случая" $
$ va(F_c) = - C (v)|v^2| * va(v), "для квадратичного случая" $
Ускорение — это вторая производная координаты по времени: $ va(a) = dv(r, t, 2) $
...
// Если представить это как систему уравнений второго порядка (относительно координат x и y), она будет выглядеть так:
// $ dv(v_x, t) = - gamma dot(r_x) $
// $ dv(v_y, t) = -g - gamma dot(r_y) $
...
$ v_y (t) = (v_y_0 + g/γ) * e^(-gamma t) - g/gamma $
Для координаты:
