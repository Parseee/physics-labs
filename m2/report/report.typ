...
== Сила сопротивления линейно зависит от скорости
...
=== Численное решение
Можно просто записать дифференциальное уравнение второго порядка для каждой из компонент $O X$ и $O Y$, а затем с помощью функции `solve_ivp` из пакета `scipy.integrate` можно получить решение искомого уравнения.

$ dot.double(x) = -k / m dot(x) $
$ dot.double(y) = - g -k / m dot(y) $

#figure(
  image("assets/linear_numeric.png", width: 67%),
  caption: "Образец рабоы программы для численного вычисления траектории.",
)
...
#figure(
  image("assets/linear_symbolic.png", width: 67%),
  caption: "Образец рабоы программы для аналитического вычисления траектории.",
)

== Сила сопротивления квадратичного зависит от скорости

// https://philosophicalmath.wordpress.com/2017/10/21/terminal-velocity-derivation/

=== Формулировка силы сопротивления:

Вектор силы сопротивления воздуха пропорционален вектору скорости квадратично и направлен противоположно ему:
$ va(F)_c = -1/2 C_d * rho va(v) / v v^2 $
где k — коэффициент сопротивления (k > 0).

=== Численное решение
Аналогично со случаем с линейным соотношением запишем дифференциальное уравнение второго порядка для каждой из компонент $O X$ и $O Y$, а затем с помощью функции `solve_ivp` из пакета `scipy.integrate` получим решение искомого уравнения.

#figure(
  image("assets/quadratic_numeric.png", width: 67%),
  caption: "Образец рабоы программы для аналитического вычисления траектории.",
)